...
import sys # to debug value by using "tf.print"
import numpy as np
...
@name_scope("debug")
def _kernel_values(Xnew, feat, kern, f, *, full_cov=False, full_output_cov=False, q_sqrt=None, white=False):
...
def _debug_LmAndA(Xnew, feat, kern, f, *, full_cov=False, full_output_cov=False, q_sqrt=None, white=False):
...
def _choleshky_matrix(feat, kern, f, *, full_cov=False, full_output_cov=False, q_sqrt=None, white=False):
...
def _debug_variance_values(Xnew, feat, kern, f, *, full_cov=False, full_output_cov=False, q_sqrt=None, white=False):
    ...
    L_tensor = L.read_value()
    LTA = tf.matmul(L_tensor, A_tiled, transpose_a=True)  # R x M x N
    tmp2 = LTA
